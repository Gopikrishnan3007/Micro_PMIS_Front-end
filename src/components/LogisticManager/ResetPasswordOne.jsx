...
const ResetPasswordOne = () => {
  ...
  useEffect(() => {
    const logisticManagerId = sessionStorage.getItem('logisticManagerid');
    if (logisticManagerId) {
      axios.get(`http://localhost:8006/portDirector/logisticManager/${logisticManagerId}`)
        .then(response => {
          setManagerData(response.data);
        })
        .catch(() => {
          setErrorMessage('Failed to fetch logistic manager data.');
        });
    } else {
      setErrorMessage('No Logistic Manager ID found in session.');
    }
  }, []);
  ...
  const handlePasswordReset = async () => {
    ...
    if (!managerData) {
      setErrorMessage('Manager data not loaded yet.');
      return;
    }

    if (username !== managerData.userName || currentPassword !== managerData.password) {
      setErrorMessage('Invalid credentials.');
      return;
    }

    const updatedManagerData = { ...managerData, password: newPassword };

    try {
      await axios.put(`http://localhost:8006/portDirector/logisticManager/${managerData.logisticManagerId}`, updatedManagerData);
  ...
  const handleCloseModal = () => {
    ...
    if (isSuccess) {
      window.location.href = '/logisticManagerDashboard'; // Redirect to dashboard on success
    } else {
